...
import com.newcell.calltext.widgets.RegistrationNotification;
...
//	    inCallNotification.addAction(R.drawable.ic_speaker_dark, "", speakerOnIntent);
	...
	public void showNotificationForMissedCall(ContentValues callLog) {
		...
		String remoteNumber = SipUri.getPhoneNumberFromApiSipString(
						callLog.getAsString(CallLog.Calls.NUMBER));
	...
	public void showNotificationForMessage(SipMessage msg) {
		...
		if (!msg.getFrom().equalsIgnoreCase(viewingRemoteFrom)) {
			CallerInfo callerInfo = CallerInfo.getCallerInfoFromSipUri(context, msg.getFullFrom());
			String from;
			if(callerInfo != null && callerInfo.contactExists) {
				from = callerInfo.name;
			} else {
				from = callerInfo.phoneNumber;
			}
//			String from = SipUri.getDisplayedSimpleContact(msg.getFullFrom()); // Old version
...
//			}
    ...
    public void showNotificationForVoiceMail(SipProfile acc, int numberOfMessages) {
        ...
        if(numberOfMessages > 1) messageText += "s";
