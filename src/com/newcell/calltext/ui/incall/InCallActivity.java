...
package com.newcell.calltext.ui.incall;
...
import com.newcell.calltext.api.MediaState;
import com.newcell.calltext.api.SipCallSession;
import com.newcell.calltext.api.SipCallSession.StatusCode;
...
import com.newcell.calltext.ui.incall.CallProximityManager.ProximityDirector;
import com.newcell.calltext.ui.incall.DtmfDialogFragment.OnDtmfListener;
import com.newcell.calltext.ui.incall.locker.IOnLeftRightChoice;
import com.newcell.calltext.ui.incall.locker.InCallAnswerControls;
import com.newcell.calltext.ui.incall.locker.ScreenLocker;
import com.newcell.calltext.utils.CallsUtils;
import com.newcell.calltext.utils.DialingFeedback;
import com.newcell.calltext.utils.Log;
import com.newcell.calltext.utils.PreferencesProviderWrapper;
import com.newcell.calltext.utils.Theme;
import com.newcell.calltext.utils.keyguard.KeyguardWrapper;
...
public class InCallActivity extends SherlockFragmentActivity implements IOnCallActionTrigger, 
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            ...
            case PICKUP_SIP_URI_NEW_CALL:
                ...
                return;
            default:
            	Log.i(THIS_FILE, "Result Code=" + resultCode
            			+ "\nRequest Code=" + requestCode
            			+ "\nIntent Action=" + data.getAction());
                break;
        }
    ...
    */
    ...
    /**
     * Perform a DTMF tone for when a call is ended
     */
    private void runEndCallDtmfTone() {

		Log.i(THIS_FILE, "Perform End Call DTMF Tone");
    	ToneGenerator toneGenerator = new ToneGenerator(AudioManager.STREAM_VOICE_CALL, 80);
    	toneGenerator.startTone(ToneGenerator.TONE_PROP_PROMPT, 600);

    	// TODO:: Since this is called twice??? we can't do .release() 
    	//        because it cuts off the next tone
    ...
    private BroadcastReceiver callStateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals(TERMINATE_CALL)) {
            	
            	Log.i(THIS_FILE, "Terminate Call from notification");
            	
            	SipCallSession callSession = intent.getParcelableExtra(SipManager.EXTRA_CALL_INFO);
            	onTrigger(TERMINATE_CALL, callSession);
            	 
            } else if (action.equals(SPEAKER_ON)) {

            	Log.i(THIS_FILE, "Speaker ON from notification");
            	
            	SipCallSession callSession = intent.getParcelableExtra(SipManager.EXTRA_CALL_INFO);
            	onTrigger(SPEAKER_ON, callSession);
            	
            } else if (action.equals(SPEAKER_OFF)) {

            	Log.i(THIS_FILE, "Speaker OFF from notification");
            	
            	SipCallSession callSession = intent.getParcelableExtra(SipManager.EXTRA_CALL_INFO);
            	onTrigger(SPEAKER_OFF, callSession);
...
//	private Timer toneTimer = null;
	...
	/**
	 * Class to generate an audible tone in its own thread
	 */
    /*
	private class ThreadedTonePlay extends Thread {
	    ...
	    public void run() {
            synchronized (toneGeneratorLock) {
                ...
                toneTimer.schedule(new StopTimerTask(), DialingFeedback.TONE_LENGTH_MS);
            }
	    }
	}
	*/
	/**
	 * Helper class for ThreadedTonePlay
	 */
	/*
