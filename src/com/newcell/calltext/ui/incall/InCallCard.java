...
import com.newcell.calltext.api.SipCallSession.MediaState;
...
//        canVideo = prefs.getPreferenceBooleanValue(SipConfigManager.USE_VIDEO);
    ...
    /*
     * Changed ratio from 0.75f to 0.85f to make endButton not as 'tall'
     */
    //---------------------------------------------------------------------------------------------
    private static float minButtonRation = 0.85f;
    //---------------------------------------------------------------------------------------------
    ...
    private void updateRemoteName() {
        ...
        if (aRemoteUri != null && !aRemoteUri.equalsIgnoreCase(cachedRemoteUri)) {
            ...
            String displayNumber = SipUri.getFormattedPhoneNumber(uriInfos); // maybe no cause we want name if contact
            ...
            /*
             * 08/05/2014
             * Changed remoteName to Voicemail if we're calling *86
             */
            if(TextUtils.equals("*86", text)) {
            	remoteName.setText(getContext().getString(R.string.voice_mail));
            }
            
            statusTextBuffer.append(uriInfos.userName);
            remoteSipAddress.setText(displayNumber);
            //remoteSipAddress.setText(statusTextBuffer.toString()); // old version
