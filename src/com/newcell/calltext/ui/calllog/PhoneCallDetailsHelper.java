...
package com.newcell.calltext.ui.calllog;
...
public class PhoneCallDetailsHelper {
    ...
    public void setPhoneCallDetails(PhoneCallDetailsViews views, PhoneCallDetails details) {
        ...
        // Display name and display number
        ...
        if (!TextUtils.isEmpty(details.name)) {
            ...
            if (!TextUtils.isEmpty(details.number)) {
            	// maybe dont use .getPhoneNumber cause we want contacts name if a contact
            	//displayNumber = SipUri.getPhoneNumber(SipUri.parseSipContact(firstDetails.number.toString()));
                displayName = SipUri.getDisplayedSimpleContact(details.number.toString());
                // SipUri.getCanonicalSipContact(details.number.toString(), false); // old version
            } else {
        ...
        /*
         * TODO::
         * I get a 403 / URI domain not configured locally, relaying forbidden sometimes when calling
         * someone from call logs list
         * NOT on every log, just some. Maybe just the missed call ones.
         */
        // Display number
        if (!TextUtils.isEmpty(details.formattedNumber)) {
        	//-------------------------------------------------------------------------------------
        	//views.numberView.setText(details.formattedNumber); // old version
            views.numberView.setText(SipUri.getFormattedPhoneNumber(SipUri.parseSipContact(details.formattedNumber.toString())));
            //-------------------------------------------------------------------------------------
        } else if (!TextUtils.isEmpty(details.number)) {
        	//-------------------------------------------------------------------------------------
            //views.numberView.setText(details.number); // old version
            views.numberView.setText(SipUri.getFormattedPhoneNumber(SipUri.parseSipContact(details.number.toString())));
            //-------------------------------------------------------------------------------------
        } else {
