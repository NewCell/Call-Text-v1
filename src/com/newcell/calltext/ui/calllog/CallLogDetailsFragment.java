...
import com.newcell.calltext.utils.ContactsAsyncHelper;
...
public class CallLogDetailsFragment extends SherlockFragment {
    ...
    private void updateData(final Uri... callUris) {
        ...
        if (!TextUtils.isEmpty(firstDetails.name)) {
        	...
        	Log.i(THIS_FILE, "Contact Number::" + firstDetails.number);
        	Log.i(THIS_FILE, "Formatted Num ::" + SipUri.getDisplayedSimpleContact(firstDetails.number));
        } else {
        	Log.i(THIS_FILE, "Contact Number::" + firstDetails.number);
        	Log.i(THIS_FILE, "Formatted Num ::" + SipUri.getDisplayedSimpleContact(firstDetails.number));
            nameOrNumber = SipUri.getDisplayedSimpleContact(firstDetails.number);
        }
        ...
        if (contactUri != null) {
            ...
            mainActionIntent = ContactsWrapper.getInstance().getAddContactIntent(
            		(String) firstDetails.name, 
            		(String) SipUri.getDisplayedSimpleContact(firstDetails.number));
        ...
        if (!TextUtils.isEmpty(firstDetails.number)) {
        	/*
        	 * Changed display number to remove the sip details
        	 */
        	//-------------------------------------------------------------------------------------
        	//displayNumber = SipUri.getPhoneNumber(SipUri.parseSipContact(firstDetails.number.toString()));
//        	displayNumber = SipUri.getFormattedPhoneNumber(SipUri.parseSipContact(firstDetails.number.toString()));
        	displayNumber = SipUri.getDisplayedSimpleContact(firstDetails.number);
        	//-------------------------------------------------------------------------------------
        } else {
    ...
    private void configureCallButton(String callText, CharSequence nbrLabel, CharSequence number) {
        ...
        //displayNumber = SipUri.getDisplayedSimpleContact(number);
